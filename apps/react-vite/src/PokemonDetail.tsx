...
export default function PokemonDetail() {
  ...
  const fetchPokemon = useCallback(async () => {
    ...
    gunsole.info({
      message: `Fetching Pokemon detail: ${pokemonId}`,
      bucket: "api_request",
      context: { pokemonId },
    ...
    try {
      ...
      if (!response.ok) throw new Error(`Pokemon not found: ${pokemonId}`);
      ...
      gunsole.error({
        message: `Failed to fetch Pokemon detail: ${pokemonId}`,
        bucket: "api_request",
        context: { pokemonId, error: errorMessage },
